...
def genFamilyParser():
    ...
    currentDic = {}
    dateType = ''
    i = 0 #bad smell, do not know what variable is for (counter, other thing?)
    for line in infile:
        if line[0] == '0':
            i = 0
            if checkIfValidTagExc(line, firstLevelExceptions) is True:
                ln = line.split()
                if ln[2] == 'INDI':
                    if ln[1].replace("@", "") in personDic.keys(): #if key already in person dictionary #bad smell , 2 things in same spot
                        i = 1
